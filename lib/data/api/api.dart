...
abstract class Api {
  ...
  static Future<PrivateResponse> loginFacebook(String tokenID) async {
    try {
      ...
      var httpRes = await http.post(Uri.parse('$_url/auth/facebook/validate'),
          headers: headers, body: body);
  ...
  static logout(String uid, String client, String token) async {
    try {
      ...
      http.delete(Uri.parse('$_url/auth/sign_out'), headers: headers);
    } catch (e) {}
  }
  ...
  static Future<PrivateResponse> getLevels(
    ...
    try {
      ...
      var httpRes = await http.get(Uri.parse('$_url/levels'), headers: headers);
  ...
  static Future<PrivateResponse> replaceExercise(User user, String client,
    ...
    try {
      ...
      var httpRes = await http.post(
          Uri.parse('$_url/exercise_variants/$exerciseID/replace'),
          headers: headers,
          body: body);
  ...
  static Future<PrivateResponse> getExerciseDetections(
      User user, String client, String token, List<String> exerciseIds) async {
    try {
      Map<String, String> headers = {
        "uid": user.email,
        "client": client,
      ...
      if (httpRes.statusCode == 200) {
        var exerciseDetections =
            ExerciseDetection.fromJsonArray(json.decode(httpRes.body));
        var newToken = httpRes.headers['access-token'];
        if (newToken == null) {
          return PrivateResponse.error(['API: token is absent in header']);
        } else {
